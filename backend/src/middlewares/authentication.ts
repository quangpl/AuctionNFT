...
export function requireJwtMiddleware(
    ...
    const decodedSession: sessionType.DecodeResult = jwt.decodeSession(
    ...
    const expiration: sessionType.ExpirationStatus = jwt.checkExpiration(
    ...
    if (expiration == "grace") {
        ...
        const { token, expires, issued } = jwt.encodeSession(
            process.env.JWT_SECRET_KEY,
            decodedSession.session,
        );
